...
export const generateLiftingPlan = (weight, experienceLevel, targetWeight, numberOfWeeks) => {
  ...
  const baseWeightFactor = experienceLevels[experienceLevel];
  const initialWeight = weight * baseWeightFactor;
  const progressiveOverloadIncrement = initialWeight * 0.025;

  const setsAndReps = {
    beginner: [[3, 8], [3, 8], [3, 8]],
    intermediate: [[4, 6], [4, 6], [4, 6]],
    advanced: [[5, 5], [5, 5], [5, 5]],
  };

  const volume = setsAndReps[experienceLevel];

  const plan = [];

  for (let t = 1; t <= numberOfWeeks; t++) {
    const weightToLift = initialWeight + t * progressiveOverloadIncrement;
    const [sets, reps] = volume[(t - 1) % volume.length];
    plan.push({
      week: t,
      weight: weightToLift,
      sets,
...
export const createLiftingPlan = async (req, res, next) => {
  ...
  try {
    ...
    const isValidExercise = fitnessKeywords.some(keyword => keyword.toLowerCase() === desiredExercise.toLowerCase());
    if (!isValidExercise) {
    ...
    const liftingPlan = generateLiftingPlan(weight, lowerCaseExperienceLevel, targetWeight, numberOfWeeks);
    if (!liftingPlan || liftingPlan.length === 0) {
      return res.status(500).json({ message: 'Failed to generate lifting plan.' });
    }

    const newLiftingPlan = {
      height,
      weight,
      experienceLevel: lowerCaseExperienceLevel,
      gender,
      desiredExercise,
      targetWeight,
      numberOfWeeks,
    ...
    return res.status(201).json({ message: 'Lifting plan created successfully', liftingPlan: newLiftingPlan });
  } catch (error) {
...
export const generateLiftingPlanResponse = async (req, res, next) => {
  const { height, weight, experienceLevel, gender, desiredExercise, targetWeight, numberOfWeeks } = req.body;

  // Validate request body and log missing fields
  const missingFields = [];
  if (!height) missingFields.push('height');
  if (!weight) missingFields.push('weight');
  if (!experienceLevel) missingFields.push('experienceLevel');
  if (!gender) missingFields.push('gender');
  if (!desiredExercise) missingFields.push('desiredExercise');
  if (!targetWeight) missingFields.push('targetWeight');
  if (!numberOfWeeks) missingFields.push('numberOfWeeks');
  ...
  // Ensure experienceLevel is a string before calling toLowerCase
  if (typeof experienceLevel !== 'string') {
    return res.status(400).json({ message: 'Experience level must be a string' });
  }
  ...
  try {
    ...
    // Generate the lifting plan using the provided function
    const liftingPlan = generateLiftingPlan(weight, experienceLevel.toLowerCase(), targetWeight, numberOfWeeks);

    // Prepare a detailed description of the lifting plan for the OpenAI prompt
    const liftingPlanDescription = liftingPlan.map((weekPlan) => {
      return `Week ${weekPlan.week}: Lift ${weekPlan.weight.toFixed(2)} lbs for ${weekPlan.sets} sets of ${weekPlan.reps} reps.`;
    }).join('\n');

    // Define the logic and parameters used in the generateLiftingPlan function
    const liftingPlanLogic = `
      The lifting plan should be generated using the following logic:
      - Height: ${height}
      - Weight: ${weight}
      - Experience Level: ${experienceLevel} (beginner: 0.5, intermediate: 0.6, advanced: 0.7)
      - Gender: ${gender}
      - Desired Exercise: ${desiredExercise}
      - Target Weight: ${targetWeight}
      - Number of Weeks: ${numberOfWeeks}
      - Initial weight factor based on experience level.
      - Progressive overload increment of 2.5% per week.
      - Sets and reps based on experience level:
        - Beginner: 3 sets of 8 reps.
        - Intermediate: 4 sets of 6 reps.
        - Advanced: 5 sets of 5 reps.
      - Each week, the weight to lift increases by the progressive overload increment.
    `;
    ...
    const messages: ChatCompletionMessageParam[] = [
      ...
      {
        role: "user",
        content: `Generate a multi-week lifting plan specifically for ${desiredExercise} based on the following details:
                  Height: ${height}, 
                  Weight: ${weight}, 
                  ...
                  Number of Weeks: ${numberOfWeeks}.
                  ${liftingPlanLogic}
                  Here is the base lifting plan:
                  ${liftingPlanDescription}`
      },
    ];

    // Log the request data for debugging
    console.log('OpenAI API request:', { model: "gpt-3.5-turbo", messages });

    // Make OpenAI API call using the chat completion endpoint
    ...
    console.log('Full OpenAI API response:', chatResponse);

    if (chatResponse.choices && chatResponse.choices.length > 0) {
      const responseMessage = chatResponse.choices[0].message?.content || '';
      res.json({ message: 'Lifting plan generated successfully', liftingPlan: responseMessage });
    } else {
      throw new Error('Unexpected API response format');
    }
  } catch (error) {
    console.error('Error generating lifting plan:', error.response ? error.response.data : error.message);
    return res.status(500).json({ message: 'Error generating lifting plan' });
  }
};